...
public class Vulture extends SmartAgentBase
{
...
// Routes can be expensive to calculate. So store any that we are going to follow.
...
protected boolean placedToKill = false;	// did we place this round with the intent to kill ?

protected void setToKillPlayer(int numberOfArmies)
	{
	// calculate some stats about players:
	int cardsWorth = board.getNextCardSetValue();
	...
	for (int i = 0; i < numPlayers; i++)
		...
		if (i == ID)
			...
			{
			// Discount their armies based on the card reward we'll get for killing them
			armies[i] -= cardsWorth*(board.getPlayerCards(i)/3.0);
			// If they are alive AND the lowest so far AND within range of us killing them remember them
			if (BoardHelper.getPlayerCountries(i, countries) > 0 
				&& armies[i] < lowestArmyCount
				&& BoardHelper.getPlayersBiggestArmyWithEnemyNeighbor(ID, countries).getArmies() + numberOfArmies > BoardHelper.getPlayerArmies(i, countries) + BoardHelper.getPlayerCountries(i, countries))
				{
	...
	if (ourArmies > lowestArmyCount * 2)
		...
		debug("We are going to try and totally kill player "+board.getPlayerName(toKillPlayer));
		}
	}
...
protected void placeToKill(int numberOfArmies)
	...
	CountryClusterSet clusters = CountryClusterSet.getAllCountriesOwnedBy( toKillPlayer, countries );
	if (clusters.numberOfClusters() == 1)
		...
		// Don't bother looking for a route in more then 20 countries, since it can hang the
		...
		if (clusterRoute != null)
			...
			// So we just need to get to one end of the path and then follow it
			Country start = clusterRoute.start();
			Country end = clusterRoute.end();

			CountryRoute routeToStart = new CountryRoute( BoardHelper.easyCostCountryWithOwner( start, ID, countries ) );
			CountryRoute routeToEnd = new CountryRoute( BoardHelper.easyCostCountryWithOwner( end, ID, countries ) );
			CountryRoute routeToCluster, attackRoute;

			if (routeToStart.costNotCountingPlayer(ID, toKillPlayer) - routeToStart.start().getArmies() 
					< routeToEnd.costNotCountingPlayer(ID, toKillPlayer) - routeToEnd.start().getArmies() )
				{
			...
			debug("We have "+placeOnCountry.getArmies()+" + "+numberOfArmies+" armies. They have "+clusterRoute.getArmies()+" armies + "+(routeToCluster.costNotCountingPlayer(ID) - routeToCluster.end().getArmies())+" needed to get there.");

			// Do we have enough armies to take out this guy this turn?
			if (placeOnCountry.getArmies() + numberOfArmies > clusterRoute.getArmies() + routeToCluster.costNotCountingPlayer(ID) - routeToCluster.end().getArmies())
				{
				placedToKill = true;
				debug("Placing all armies on "+placeOnCountry);
				board.placeArmies( numberOfArmies, placeOnCountry );
...
public void attackPhase()
	...
	// Operate in a loop in case we eliminate for some cards and have the chance to take out another player afterwards
	while (attackRoutes.size() > 0)
		{
		CountryRoute attackRoute = (CountryRoute) attackRoutes.remove(0);

		debug("We are attacking to kill off the player along the route: "+attackRoute);
...
protected boolean attackAlongRoute(CountryRoute route)
	...
	for (int i = 1; i < route.size(); i++)
		{
		((SmartAgentBase)backer).moveInMemory = 1000000;
		if (route.get(i-1).getArmies() > 1 && route.get(i-1).canGoto(route.get(i)) && route.get(i-1).getOwner() == ID)
			{ // attack...
			if (board.attack(route.get(i-1), route.get(i), true) == 13)
				{
				// then we lost
				((SmartAgentBase)backer).moveInMemory = -1;
				return false;
				}
			}
		}
