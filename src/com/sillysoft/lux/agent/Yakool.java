...
public class Yakool extends Cluster 
{
...
protected void attackFromCluster( Country root )
	...
	//	1. if we own zero continents	(hopefully we will take over a cont then)
	//	2. our income is below average
	//	3. our income is the highest	(hopefully we will take over the world then)
	// 	4. we outnumber all the other player combined	(hopefully we will take over the world then)
	if (! BoardHelper.playerOwnsAnyPositiveContinent( ID, countries, board ) 
		|| incomes[ID] < aveEnemyIncome 
		|| incomes[ID] > maxEnemyIncome 
