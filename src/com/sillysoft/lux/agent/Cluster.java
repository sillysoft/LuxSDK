...
//
//  Cluster is a concrete agent built using SmartAgentBase. He expands from his biggest cluster of countries.
//	In particlular...
//
//		He picks countries in continents decided by the pickGoalContChoosing() method.
//			This method chooses medium sized continents, starting with fully unowned ones.
//
//		He places armies evenly along the border of the cluster centered on the biggest owned continent
//			or centered on the his biggest army if no continents are owned
//
...
//
//	Cluster doesn't really care about continents. He only thinks about them when choosing his initial countries.
//		He doesn't try to take them over or defend them.
//


public class Cluster extends SmartAgentBase 
{
public String name()
	{
	return "Cluster";
...
// Then use the SmartAgentBase method pickCountryInContinent() to select the best country.
public int pickCountry( )
	{
	// If we have yet to choose a goal or our goal is fully owned then choose a goal
...
This method examines all the continents to see which one we should try and take over
at the very beginning of the game.

It returns the continentCode of the continent it decides on.

For now it picks the continent that has the most countries but not over <max>, 
where <max> is dependant on the number of countries and players.
It checks for fully unowned conts first, then any goes on to any conts
*/
protected int pickGoalContChoosing()
	{
	int max = (int)Math.ceil((numCountries*1.25)/board.getNumberOfPlayers());

	// First of all we look at all the continents, and choose the biggest one (but not over <max>)
	// that is FULLY empty
	...
	for (int cont = 0; cont < numContinents; cont++) {
		if (board.getContinentBonus(cont) > 0)
			...
			if (size < max && size > bigUnownedContSize && BoardHelper.playerOwnsContinent( -1, cont, countries ))
				{
	...
	// We choose the biggest continent under <max> that has ANYTHING unowned.
	...
	for (int cont = 0; cont < numContinents; cont++) {
		if (board.getContinentBonus(cont) > 0)
			{
			int size = BoardHelper.getContinentSize(cont, countries);
			if (size < max && size > bigUnownedContSize && BoardHelper.playerOwnsContinentCountry(-1, cont, countries )) {
				bigUnownedContSize = size;
				bigUnownedCont = cont;
				}
			}
...
// We only get to here if all the continents that are left with less than <max> countries are all taken.
...
// we pick the smallest one that has something to pick:
	...
	for (int cont = 0; cont < numContinents; cont++)
		{
		int size = BoardHelper.getContinentSize(cont, countries);
		if (size >= max && size < smallestSize && BoardHelper.playerOwnsContinentCountry(-1, cont, countries ))
			{
...
// Use the SmartAgentBase method placeArmiesOnClusterBorder() to place our armies evenly along a cluster.
...
public String youWon()
	...
	"Silly little muggles",
	"My middle name is Beowulf",
	"Clusta da Busta",
	"Cluster knows how to muster his troops",
	"I declare today a global day of funk",
	"Dude! That was sweet!" };
